...
class EventKickView(APIView):
    ...
    @extend_schema(
        tags=["약속관리"], summary="멤버 강퇴", request=EventMemberManageSerializer
    )
    def post(self, request, event_id):
        ...
        # 3. 권한 부여 서비스 로직 요청
        success = EventMemberService.kick_user(event_id, target_user_id, request.user)
        ...
        # 5. 거절된 경우
        return Response(
            {"message": "강퇴는 방장만 가능합니다."}, status=status.HTTP_403_FORBIDDEN
        )
